# Klub Medyka — Design System

**Źródło prawdy dla warstwy wizualnej projektu.** Ten dokument definiuje tokeny, komponenty i konwencje, które obowiązują w całym repo.

---

## O tym dokumencie

**Zakres**: ten design system dotyczy **strony marketingowej / sklepu produktowego** (bazuje na projekcie Figma "Klub Medyka"). Stack: React + Tailwind/CSS + komponenty shadcn/ui.

**Nie dotyczy**: prototypu dashboardu członkowskiego (`klub-medyka-clean.jsx` / `beta-1/`) — tam obowiązuje odrębny system opisany w [`SPEC.md`](./SPEC.md) §2 (DM Sans + inline styles + inna paleta). Nie mieszać tych dwóch systemów.

## Jak używać (Claude + developer)

- **Tokeny są źródłem prawdy** — nie wymyślaj nowych kolorów, rozmiarów, radiusów. Jeśli czegoś brakuje — dodaj token tutaj, nie hardkoduj.
- **Font**: jeden — `Inter`. Nic innego.
- **Brand colors nietykalne**: `#2E35FF` (Remedium Blue) i `#1E1926` (Remedium Dark). Nie zmieniać nawet punktowo.
- **Komponenty**: najpierw sprawdź shadcn/ui. Jeśli jest odpowiednik — użyj go. Jeśli nie — dobuduj w stylu shadcn (compound components, `cn()` utility, warianty przez `cva`).
- **CSS variables**: korzystaj z sekcji 15 zamiast hardkodować wartości.
- **Spacing**: tylko z tokenów (baza 4px). `padding: 13px` nie istnieje.
- **Radius**: 4 warianty (md/lg/xl/full). Nie interpoluj między nimi.

## Czego NIE robić

- Nie wprowadzać drugiego fontu
- Nie mieszać systemu shadcn z systemem prototypu dashboardu
- Nie dodawać nowych brand colors bez uzgodnienia
- Nie robić migracji na inny framework CSS bez rozmowy (Tailwind jako utility jest OK, ale nie rewrite całości)
